...
(async () => {
  ...
  const renderNotes = async (notes) => {
    ...
    <ul>
     ${notes
       .map(
         (note) =>
           `<li data-note-id='${note._id}'>${note.content} <button>important: ${note.important}</button></li>`
       )
       .join("")}
    </ul>
    `;
  ...
  root.addEventListener("click", async (event) => {
    ...
    const noteTobeChanged = notes.find((note) => note._id == noteId);
